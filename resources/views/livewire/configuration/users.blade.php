<div>
    ...
    <div class="mb-4">
        <!-- bread -->
        <x-custom.bread>
            @slot('category') Configuración  @endslot
            Usuarios
        </x-custom.bread>
         <!-- page title -->
         <h1 class="mt-2 text-2xl font-extrabold leading-none tracking-tight text-gray-900 md:text-2xl lg:text-3xl dark:text-white">Usuarios</h1>
    </div>
    ...
    <x-modal blur wire:model="openNew">
        <x-card title="Nueva sucursal">
                ...
                <div class="grid gap-6 mb-6">
                    <div>
                        <x-input label="Contraseña" placeholder="Contraseña" wire:model.defer="password" />
                    </div>
                </div>
                <x-slot name="footer">
                    <div class="flex justify-end gap-x-4">
    ...
    <x-modal blur wire:model="openEdit">
        <x-card title="Editar usuario">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <x-input label="Nombre" placeholder="Nombre de la sucursal" wire:model.defer="user.name" />
                </div>
                <div>
                    <x-input label="email" placeholder="correo electronico" wire:model.defer="user.email" />
                </div>
            </div>
            <div class="grid gap-6 mb-6">
                <div>
                    <x-inputs.password label="password" placeholder="Contraseña" wire:model.defer="new_password" />
                </div>
            </div>
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
