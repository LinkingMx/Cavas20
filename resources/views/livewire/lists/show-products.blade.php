<div>
    ...
    <div class="items-center justify-between block sm:flex md:divide-x md:divide-gray-100 dark:divide-gray-700">
        ...
        <button id="createProductButton" wire:click="$set('openNewProduct', true)" class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800" type="button" data-drawer-target="drawer-create-product-default" data-drawer-show="drawer-create-product-default" aria-controls="drawer-create-product-default" data-drawer-placement="right">
            Nuevo producto
        </button>
    </div>
    <!-- end header -->
    ...
    <x-modal blur wire:model="openUploadProducts">
    ...
    <!-- New product Modal -->
    <x-modal blur wire:model="openNewProduct">
        <x-card title="Nuevo producto">
                <x-errors class="mb-4" />
                <div class="grid gap-6 mb-6 md:grid-cols-2">
                    <div>
                        <x-inputs.number label="Codigo SAP" wire:model.defer="sap" />
                    </div>
                    <div>
                        <x-inputs.currency
                        label="Precio"
                ...
                <x-slot name="footer">
                    <div class="flex justify-end gap-x-4">
                        <x-button flat label="Cancel" wire:click="$set('openNewProduct', false)" />
    ...
    <!-- Edit product Modal -->
    <x-modal blur wire:model="openEditProduct">
        <x-card title="Editar producto">
            <x-errors class="mb-4" />
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <x-inputs.number label="Codigo SAP" wire:model="product.sap" />
                </div>
                <div>
                    <x-inputs.number label="Precio" wire:model="product.price" />
                </div>
            </div>
            <div class="mb-6">
                <x-input label="Nombre" placeholder="nombre del producto" wire:model="product.name" />
            </div>
            <x-slot name="footer">
                <div class="flex justify-end gap-x-4">
                    <x-button flat label="Cancel" wire:click="$set('openEditProduct', false)" />
